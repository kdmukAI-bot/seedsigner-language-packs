...
Produces a production-ready per-locale font pack under the build output dir:
  build/<locale>/<locale>.ttf  +  manifest.json (sha256)

Per locale:
  1. Read the locale->{font,chain,shaping,endonym} policy from the master config
     `locales.h` (via locales_h.py) -- the single source of truth this repo owns
     and screens vendors. It needs each locale's source font family, the locale
     SET (--locale restricts it), and the endonym + its per-height button px.
     (Replaces the former `screenshot_gen --dump-locales` dependency: the builder
     no longer compiles the render layer to learn policy.)
...
This is the single entry point for font packs. Self-contained: reads policy from
locales.h, parses .po directly (no Babel), subsets via fontTools. Reproducible
UNSIGNED output; signing is a separate, offline, per-locale step (see README).

Prerequisites: the pinned pack-builder Docker image (fontTools + uharfbuzz + PyICU
+ Pillow/libraqm + lv_shape), or those installed locally. Run via scripts/build_packs.sh.
"""
...
# This file lives at tools/; the repo root is one level up.
...
def ensure_shaper(shaper_bin):
    ...
    lv_shape (tools/shaper) is the LVGL Arabic/Persian shaping oracle; it must link
    the SAME LVGL the consumers render with (v9.5.0), so we build it from in-repo
    source. This repo has no third_party/lvgl submodule — the pinned LVGL checkout
    is supplied via $LVGL_ROOT (set in the pack-builder Docker image) and passed
    straight through to CMake."""
    if os.path.exists(shaper_bin):
        return shaper_bin
    src_dir = os.path.join(REPO_ROOT, "tools/shaper")
    ...
    lvgl_root = os.environ.get("LVGL_ROOT")
    if lvgl_root:
        cmake_cmd.append(f"-DLVGL_ROOT={lvgl_root}")
...
def source_ttf(source_family, assets_dir):
    ...
    (vendored in fonts/). CJK Noto families ship a single Regular weight, which
    serves every text role; the OpenSans block-range packs use both weights."""
...
def _run_subset(src_ttf, unicodes, out_ttf, drop_layout):
    cmd = [
        ...
        "--notdef-outline",
        # Reproducibility pin: never stamp head.modified with "now" (the #1 churn
        # risk). This is fontTools' current default, but we pin it explicitly so a
        # future default flip can't silently make the signed bytes non-reproducible.
        "--no-recalc-timestamp",
    ]
    if drop_layout:
...
# Locale policy (source_family / chain / unicode_range / shaping / script / rtl),
# the per-locale endonym string, and the per-height endonym button px all come from
# the master config locales.h via locales_h.py:
#   locales_h.manifest_locales(config, only)   — the policy dict per locale
#   locales_h.endonyms(config)                 — native display names
#   locales_h.endonym_sizes_by_locale(config)  — [(height, button_px), ...] per locale
# (These replaced the former `screenshot_gen --dump-locales` + supported_locales.json
# readers; the reader is verified against that dump's output in CI.)
...
def main():
    ...
    ap.add_argument("--config",
                    default=os.path.join(REPO_ROOT, "locales.h"),
                    help="master locale policy (locales.h); replaces screenshot_gen --dump-locales")
    ap.add_argument("--translations-dir",
                    default=os.path.join(REPO_ROOT, "seedsigner-translations"),
                    help="seedsigner-translations checkout (.po catalogs)")
    ap.add_argument("--assets-dir",
                    default=os.path.join(REPO_ROOT, "fonts"),
                    help="vendored source TTFs (OpenSans + Noto), owned by this repo")
    ap.add_argument("--out-dir",
                    default=os.path.join(REPO_ROOT, "build"),
                    help="output root for <locale>/<locale>.ttf (gitignored on-the-fly build)")
    ap.add_argument("--locale", action="append",
                    help="only build this locale (repeatable); default = all in locales.h")
    ap.add_argument("--shaper-bin",
                    default=os.path.join(REPO_ROOT, "tools/shaper/build/lv_shape"),
    ...
    ap.add_argument("--catalogs-only", action="store_true",
                    help="(re)compile just the .mo catalogs — embed into existing packs + emit "
                         ...
                         "HarfBuzz/ICU/locales.h reader; translations change more often than fonts)")
    args = ap.parse_args()
    ...
    locales = locales_h.manifest_locales(args.config, args.locale)
    if not locales:
        print("No matching locales in locales.h.", file=sys.stderr)
        return 1
    ...
    # runtime font; both the strings and the sizes come from locales.h.
    endonyms = locales_h.endonyms(args.config)
    endonym_sizes = locales_h.endonym_sizes_by_locale(args.config)
