...
This is a DEV/QA oracle, not part of the reproducible pack build: it needs an
external `screenshot_gen --shape-spike` binary (built in the seedsigner-lvgl-screens
repo — this repo intentionally does not build the render layer). Pass it via
--gen-bin or the SEEDSIGNER_SCREENSHOT_GEN env var.

Usage:
    python tools/validate_runs.py --gen-bin /path/to/screenshot_gen --locale hi [--locale th ...]
    python tools/validate_runs.py --gen-bin /path/to/screenshot_gen --all-locales
Options:
...
def validate_locale(locale, font_dir, want_all, gen_bin):
    ...
    # Device render through the proven glyph-id path (external screens binary).
    subprocess.run([gen_bin, "--shape-spike", out_dir], check=True,
...
def main():
    ...
    ap.add_argument("--font-dir", default=os.path.join(REPO_ROOT, "build"),
                    help="pack build output dir holding <locale>/runs.json + subset .ttf")
    ap.add_argument("--gen-bin", default=os.environ.get("SEEDSIGNER_SCREENSHOT_GEN"),
                    help="external screenshot_gen --shape-spike binary (from seedsigner-lvgl-screens)")
    ...
    if not args.gen_bin or not os.path.exists(args.gen_bin):
        print("ERROR: need an external screenshot_gen --shape-spike binary; pass --gen-bin "
              "or set SEEDSIGNER_SCREENSHOT_GEN (built in the seedsigner-lvgl-screens repo).",
              file=sys.stderr)
        return 1

    overall = True
    for loc in locales:
        overall &= validate_locale(loc, args.font_dir, args.msgids_all, args.gen_bin)
